...
def initializeModel(output_channels, pretrained, name, set_train=True):
    print(f"Selected architecture:  {name}")
    model = None
    ...
    if name == 'Deeplab101' and pretrained is False:
        model = models.segmentation.deeplabv3_resnet101(pretrained=pretrained, num_classes=output_channels,
                                                        progress=True)
    ...
    if name == 'MobileNetV3-Large' and pretrained is False:
        model = models.segmentation.deeplabv3_mobilenet_v3_large(pretrained=pretrained, num_classes=output_channels,
                                                                 progress=True)
    ...
    if name == 'LR-ASPP-MobileNetV3-Large' and pretrained is True:
        ...
        model = models.segmentation.lraspp_mobilenet_v3_large(pretrained=pretrained, progress=True)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for sample in tqdm(iter(dataloaders['Train'])):
            ...
            with torch.set_grad_enabled(True):
                ...
                # print(f"DEBUG device: {devicetype}, {type(outputs)}, {type(outputs['out'])}")
                if devicetype == 'cpu':
...
def call_main(parse_these_args=None):
    parser = argparse.ArgumentParser(prog='Training', description='Script which trains a selected model')
    ...
    args = None
    if parse_these_args is None:
        args, unknown = parser.parse_known_args()
    else:
        print(parse_these_args)
        args, unknown = parser.parse_known_args(parse_these_args)
