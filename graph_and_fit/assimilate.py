...
def getallcsvs(path, ignore_old=True):
    ...
    if ignore_old:
        allcsvs = [cfile for cfile in allcsvs if not "_old" in cfile]
    return allcsvs
    ...
    for folder in expt_folders:
        ...
        for subdir in subdirs:
            if calculation_type == "training":
                ...
                print("calcsubdir: ", calcsubdir, len(calcsubdir) > 1)
                if len(calcsubdir) > 1:
                    print(len(calcsubdir))
...
def compile_folder(experiment_folders, model_substring, calculation_type="inference"):
    ...
    for folder in experiment_folders:
        ...
        for subdir in subdirs:
            ...
            csvs = getallcsvs(calcsubdir)
            print("csvs", csvs)
            combined = pd.concat([pd.read_csv(f).assign(channel=channel) for f in csvs], ignore_index=True)

            # for c, csvfile in enumerate(csvs):
            #     print(c, csvfile, flush=True)
            #     fname = os.path.basename(csvfile)
            #     modelname, _, sampletype, instrument, lr, pretrained, _metricscsv = fname.split("_")
            #     print(modelname, _, sampletype, instrument, lr, pretrained, _metricscsv)
            ...
            #             channel, modelname, sampletype, instrument, float(lr), pretrained.__contains__(
...
def compile_folder_train(experiment_folders, model_substring):
    # columns = ['channel', 'Model',	'Pretrained'	'LR'	'Batch_Size'	'epoch'	'Seconds'	'Train_loss'	'Test_loss'	'Per-Pixel Accuracy'	\
    #             'Precision_macro'	'Precision_micro'	'Recall_macro'	'Recall_micro'	'Dice_macro'	'Dice_micro'	'Jaccard_macro'	'Jaccard_micro'	\
    #             'confidence_sd_macro'	'confidence_sd_micro'	'MSE']
    # combined = pd.DataFrame(columns=columns)
    combined = None
    for folder in experiment_folders:
        channel = os.path.basename(folder)
        ...
        for subdir in subdirs:
            ...
            print(len(csvs), subdir)
            combined = pd.concat([pd.read_csv(f).assign(channel=channel) for f in csvs], ignore_index=True)
            # for csvfile in csvs:
            #     fname = os.path.basename(csvfile)
            #     print(fname.split("_"))
            #     try:
            #         modelname, _, sampletype, lr, pretrained = fname.split("_")
            #     except:
            #         modelname, _, sampletype, lr, pretrained = fname.split("_")
            #     print(modelname, _, sampletype, lr, pretrained)
            #     lr = lr[2:]
            #     data = pd.read_csv(csvfile, index_col=False)
            #     rows = len(data.index)
            #     for row in range(rows):
            #         combined.loc[len(combined), combined.columns] = \
            #             channel, modelname, sampletype, float(lr), pretrained.__contains__(
            #                 'True'), \
