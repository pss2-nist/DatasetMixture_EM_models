...
def plot_inference_data(df_name, title, savepath, metric='Dice_macro', xaxis_key="channel"):
    df = pd.read_excel(df_name)
    print(df.head())
    print(df[metric])
    idx = df.groupby("channel")[metric].idxmax()
    selected_df = df.loc[idx]
    print(selected_df.head())
    print(selected_df.shape)
    print(selected_df["channel"])
...
if __name__ == "__main__":
    # path = "C:/Users/pss2/PycharmProjects/DatadrivenEM/data/measured_SEM_images/Synthetic_DDS"
